...
The results of the tests are plotted on an appropriate scale once they have completed and the data is also used to fit the measured data to various models for the different devices. Fitting of the measured data is accomplished through least squares analysis and this was initially implemented using cmpfit - a C implementation of the Levenberg–Marquardt algorithm. A variety of valve models exist, often driven by a desire to represent devices is Spice simulations. For a given grid voltage, the simple triode model(1) gave good results but it is not good for representing the result sets across a range of grid voltages. Following the work of Derk Reefman (author of the uTracer), I moved to the Koren triode model(2) but found that cmpfit struggled to give consistent results for all the parameters of the model.

After a trawl of the Internet for alternative to cmpfit, I decided to look at Ceres Solver as a more complete library for optimisation and embarked on the journey of getting the library to compile for a Windows solution using Visual Studio Community. Ceres Solver has a number of dependencies, some mandatory and some optional - a minimal build requires glog and gflags (from Google) and Eigen3. Eigen3 offers good optimisation options without additional libraries but can be enhanced by enabling SuiteSparse for the linear algebra. That, however, starts to make things more complicated as there are then depedencies on some Fortran routines that obviously require a suitable Fortran compiler. One perfectly viable option for the build is to use GCC with MinGW - but that isn't a currently supported build option. Casting around for potential Fortran compilers, the Intel Fortran compiler provided by oneAPI seemed like an obvious choice. The fly in the ointment (for now) proved to be that the integration with VS Community 2019 has a known issue that is expected to be fixed by one of a) an upcoming release of the Intel compilers, or b) the imminent release of VS2021.

As a consequence of the issues with compiling SuiteSparse, I have currently integrated a minimal configuration of Ceres Solver that only depends on Eigen3. This involves using CMake to manage the builds and then referencing the libraries in the build of the QT code - the references are relative but these libraries still need to be in the right place (relatively) for the build to work. I build in <X>:\Development\projects\<ProjectName> and I install CMake libraries to <X>:\Development\CMake - the relative reference in the QT project files is thus prefixed ../../CMake/<xxx> for the required libraries. If your set up is different you will need to adjust the paths appropriately! There is a separate Git repo (with build instructions) for building a Windows version of Ceres Solver using CMake - once the update to the Intel Fortran compilers is posted and/or with the release of VS2022, I will atttempt to include the SuiteSparse library to enhance the solver options.

Initially, the default configuration of Ceres Solver yielded identical results to cmpfit. This was welcom in many respects as it confirmed that the library was doing what it was supposed to do, but less exciting in terms of giving a better result. A little playing with the available options, though, did yield a more reliable convergence to a consistent set of parameter values. Note that Derk Reefman's analysis does suggest that a) the Koren parameter Kvb does not have a significant impact on the fit (and so tends to vary considerably), and b) Kg has a similar effect to Kp in the Koren equation. In practice, using the Ceres Solver with the Linear Solver set to CGNR with suitable initial paramter values gave more consistent (and slightly more believeable) results than the Levenberg-Marquardt solution obtained with cmpfit.
...
Ceres Solver library used with Eigen3, SuiteSparse, glog and gflags
